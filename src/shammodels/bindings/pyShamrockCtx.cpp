...
using PatchData = std::vector<float>;

template<class Tdata>
using node_input_fct= std::function<Tdata(void)>;
...
class ISPHSetupNode{
    ...
    virtual bool done() = 0;
    virtual Tdata next_n(u32 nmax) = 0;
...
class EvenGenerator : public ISPHSetupNode<PatchData>{
    u32 cnt = 0;
    u32 Nmax = 0;
    public:
    EvenGenerator(u32 nmax) : Nmax(nmax) {}

    bool done(){
        return cnt >= Nmax;
    }

    PatchData next_n(u32 nmax){
        PatchData ret;
...
class Multiplier : public ISPHSetupNode<PatchData>{
    float factor = 0;
    std::shared_ptr<ISPHSetupNode<PatchData>> parent;

    public:

    Multiplier(std::shared_ptr<ISPHSetupNode<PatchData>> parent,float factor) 
        : factor(factor), parent(parent) {}
    ...
    PatchData next_n(u32 nmax){
        PatchData ret = parent->next_n(nmax);
        for(auto & a : ret){
            a *= factor;
        }
        return ret;
...
Register_pymod(pytestinit) {


    py::class_<ISPHSetupNode<PatchData>, std::shared_ptr<ISPHSetupNode<PatchData>>>(m, "ISPHSetupNode<PatchData>")
        .def("next_n", [](std::shared_ptr<ISPHSetupNode<PatchData>> & self, u32 nmax){
            return self->next_n(nmax);
        });

    m.def(
            "make_even_generator",
            [](u32 cnt) {
                return std::shared_ptr<ISPHSetupNode<PatchData>> (new EvenGenerator(cnt));
            });
    m.def(
            "make_multiplier",
            [](std::shared_ptr<ISPHSetupNode<PatchData>> & parent,float factor) {
                return std::shared_ptr<ISPHSetupNode<PatchData>> (new Multiplier(parent,factor));
            });
