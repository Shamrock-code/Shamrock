...
def print_setup_logs(filepath, name_png_prefix):
    ...
    max_msg_size = np.max([np.max([indices_size for _, _, indices_size in step['msg_list']] + [0]) for step in data])
    ...
    for step_idx, step in enumerate(data):
        ...
        ax2.barh(np.arange(world_size), step['count_per_rank'], height=0.8, color='steelblue', alpha=0.7)
        ...
        im = ax1.imshow(comm_matrix_masked, cmap=cmap, aspect='equal', interpolation='nearest', norm=norm)
        ax1.set_xlabel('Receiver Rank', fontsize=12)
        ax1.set_title('Communication Matrix & Particle Count per Rank (log scale)', fontsize=14, pad=10)
        ax1.grid(False)
