 ...
 *   <nu_i * dS_i/dt> = -Sum_j P*_ij V^2_ij,interp [nabla_i W(x_i-x_j, sqrt(2)*h_i) - nabla_j
 * W(x_i-x_j, sqrt(2)*h_j)] <nu_i * de_i/dt> = -Sum_j P*_ij v*_ij . V^2_ij,interp [nabla_i W -
 * nabla_j W]
 *
 * where (Kitajima Eq. 365):
 *   V^2_ij,interp = (1/2)(V^2_ij(h_i) + V^2_ij(h_j))  - single interpolated volume
 *   V = nu/N (volume per particle)
 *   [nabla_i W - nabla_j W] = antisymmetric gradient difference
 *
 * Note: This uses the INTERPOLATED V^2 formulation from the paper,
 * NOT the separate V^2*Omega grad-h corrected form.
 */
...
namespace shammodels::gsph::physics::sr {
    ...
    /**
     * @brief Compute SR force contribution for a particle pair (interpolated V^2 formula)
     *
     * Kitajima et al. (2025) Eq. 371 with variable smoothing length:
     *   dS_i/dt += -P* * V^2_ij,interp * (nabla W_i + nabla W_j)
     *
     * where V^2_ij,interp = 0.5 * (V_i^2 + V_j^2) is the interpolated volume factor.
     *
     * Note: The gradient signs are such that nabla W_i and nabla W_j both point in
     * the same direction (from j to i), so their sum gives the antisymmetric
     * form [nabla_i W - nabla_j W] from the paper (since nabla_j W points opposite to nabla_i W).
     *
     * @tparam Tscal Scalar type
     * @tparam Tvec Vector type
     * @param P_star Interface pressure from Riemann solver
     * @param V_i Volume of particle i (nu_i / N_i)
     * @param V_j Volume of particle j (nu_j / N_j)
    ...
    inline Tvec sr_momentum_force(
        ...
        // Kitajima Eq. 371: Force with interpolated V^2
        // F = -P* * V^2_interp * (nabla W_i + nabla W_j)
     ...
     *   dS_i/dt += -P* * (V_i^2/Omega_i * nabla W_i + V_j^2/Omega_j * nabla W_j)
     *
     * where Omega = 1 + (h/(3*rho)) * Sum_j m_j dW/dh is the grad-h correction factor.
    ...
    inline Tvec sr_momentum_force(
        ...
        // Grad-h corrected force: F = -P* * (V_i^2/Omega_i * nabla W_i + V_j^2/Omega_j * nabla W_j)
     ...
     *   de_i/dt += -P* v* . (V_i^2 Omega_i nabla W_i + V_j^2 Omega_j nabla W_j)
     *
     ...
     * V_i = nu_i / N_i
     *
     * where nu_i is the SPH particle number (related to smoothing length)
     * and N_i is the lab-frame baryon density.
     *
     * @tparam Tscal Scalar type
     * @param nu SPH particle number (= n_ngb / kernel_norm for adaptive h)
     * @param N Lab-frame density (= gamma * n, where n is rest-frame density)
     ...
     * @param gamma_lor Lorentz factor gamma
    ...
    /**
     * @brief Full SR force computation for a particle pair (interpolated V^2 formula)
     *
     * Computes both momentum and energy contributions in one call.
     * Uses Kitajima et al. (2025) Eq. 371-372 with interpolated V^2:
     *   dS_i/dt = -P* * V^2_interp * (nabla W_i + nabla W_j)
     *   de_i/dt = v* . dS_i/dt
     *
     * @tparam Tscal Scalar type
     * @tparam Tvec Vector type
    ...
    inline void sr_pairwise_force(
        ...
        // Compute momentum force using Kitajima interpolated V^2 formula
    ...
    inline void sr_pairwise_force(
        ...
        // Compute momentum force using grad-h corrected V^2/Omega formula
        Tvec force = sr_momentum_force<Tscal, Tvec>(
            P_star, V_i, V_j, omega_i, omega_j, grad_W_i, grad_W_j);
     ...
     * Uses Kitajima interpolated V^2 formulation.
     *
     * @tparam Tscal Scalar type
     * @tparam Tvec Vector type
     */
