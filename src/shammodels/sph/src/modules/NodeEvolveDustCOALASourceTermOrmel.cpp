...
/**
 * @file NodeEvolveDustCOALASourceTermOrmel.cpp
...
#include "shammodels/sph/modules/NodeEvolveDustCOALASourceTermOrmel.hpp"
...
namespace shammodels::sph::modules {
    ...
    struct KernelGenCoala_k0_Ormel {
        ...
        auto operator()(
            ...
            const Tscal *__restrict ts,

            const Tvec *__restrict a_ext,
            ...
            Tscal pmass      = this->pmass;
            Tscal mu         = this->mu;
            Tscal mh         = this->mh;
            Tscal kb         = this->kb;
            Tscal alpha_turb = this->alpha_turb;
            ...
            return [=, nbins = this->nbins](sycl::handler &cgh) {
                ...
                cgh.parallel_for(range, [=](sycl::nd_item<1> tid) {
                    ...
                    Tscal cs_a    = cs[id_a];
                    Tscal a_ext_a = sycl::length(a_ext[id_a]);
                    Tscal h_a     = hpart[id_a];
                    Tscal P_a     = pressure[id_a];

                    // TODO use true gas density instead of combined dust + gas
                    Tscal rho_gas_a = shamrock::sph::rho_h(pmass, h_a, Kernel::hfactd);

                    auto temperature = [](Tscal P, Tscal rho, Tscal mu, Tscal mh, Tscal kb) {
                        return mu * mh * P / (rho * kb);
                    };

                    Tscal T_a = temperature(P_a, rho_gas_a, mu, mh, kb);
                    ...
                    auto dv = [&](int i, int j) {
                        ...
                        // Ormel model for differential velocities
                        Tscal ts_1    = 0.;
                        Tscal St_1    = 0.;
                        Tscal St_2    = 0.;
                        Tscal x_St    = 0.;
                        ...
                        beta_St
                            = 3.2 - (1. + x_St)
                              + 2. / (1. + x_St) * (1. / 2.6 + sycl::pow(x_St, 3) / (1.6 + x_St));

                        if (ts_1 < t_eta) {

                            if (sham::abs(St_1 - St_2) < eps_diff) {
                                res = 0.;
                            } else {
                                res = alpha_turb * sycl::pow(cs_a, 2) * (St_1 - St_2)
                                      / (St_1 + St_2)
                                      * (sycl::pow(St_1, 2) / (St_1 + 1. / sycl::sqrt(Re))
                                         + sycl::pow(St_2, 2) / (St_2 + 1. / sycl::sqrt(Re)));
                            }

                        } else if ((t_eta <= ts_1) && (ts_1 < t_L)) {

                            res = alpha_turb * sycl::pow(cs_a, 2) * beta_St * St_1;

                        } else {

                            res = alpha_turb * sycl::pow(cs_a, 2)
                                  * (1. / (St_1 + 1.) + 1. / (St_2 + 1.));
                        }

                        return sycl::sqrt(res);
                    };
    ...
    template<class Tvec, template<class> class SPHKernel>
    inline void NodeEvolveDustCOALASourceTermOrmel<Tvec, SPHKernel>::_impl_evaluate_internal() {
        ...
        auto pressure_spans = edges.pressure.get_spans();
        auto cs_spans       = edges.cs.get_spans();
        auto ts_spans       = edges.ts.get_spans();
        auto a_ext_spans    = edges.a_ext.get_spans();
        ...
        Tscal alpha_turb                              = edges.alpha_turb.value;
        Tscal mu                                      = edges.mu.value;
        Tscal mh                                      = edges.mh.value;
        Tscal kb                                      = edges.kb.value;
        Tscal pmass                                   = edges.gpart_mass.value;
        ...
        counts.for_each([&](u64 id_patch, u64 count) {
            ...
            sham::kernel_call_hndl(
                ...
                sham::MultiRef{
                    ...
                    pressure_spans.get(id_patch),
                    cs_spans.get(id_patch),
                    ts_spans.get(id_patch),
                    a_ext_spans.get(id_patch)},
                sham::MultiRef{S_coag_spans.get(id_patch)},
                count,
                KernelGenCoala_k0_Ormel<Tvec, SPHKernel>{
                    nbins,
                    rho_eps,
                    corrected_len,
                    group_size,
    ...
    template<class Tvec, template<class> class SPHKernel>
    inline std::string NodeEvolveDustCOALASourceTermOrmel<Tvec, SPHKernel>::_impl_get_tex() const {
...
template class shammodels::sph::modules::NodeEvolveDustCOALASourceTermOrmel<f64_3, M4>;
template class shammodels::sph::modules::NodeEvolveDustCOALASourceTermOrmel<f64_3, M6>;
template class shammodels::sph::modules::NodeEvolveDustCOALASourceTermOrmel<f64_3, M8>;

template class shammodels::sph::modules::NodeEvolveDustCOALASourceTermOrmel<f64_3, C2>;
template class shammodels::sph::modules::NodeEvolveDustCOALASourceTermOrmel<f64_3, C4>;
template class shammodels::sph::modules::NodeEvolveDustCOALASourceTermOrmel<f64_3, C6>;
