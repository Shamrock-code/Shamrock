...
/**
 * @file SumFluxDust.cpp
 * @author Timothée David--Cléris (tim.shamrock@proton.me)
 * @brief Sum the fluxes into the time derivative fields for Dust
 *
 */
...
struct KernelSumFluxDust {
    ...
    inline void operator()(
        ...
        add_flux(graph_iter_xp, flux_rho_face_xp, flux_rhov_face_xp);
        add_flux(graph_iter_xm, flux_rho_face_xm, flux_rhov_face_xm);
        add_flux(graph_iter_yp, flux_rho_face_yp, flux_rhov_face_yp);
        add_flux(graph_iter_ym, flux_rho_face_ym, flux_rhov_face_ym);
        add_flux(graph_iter_zp, flux_rho_face_zp, flux_rhov_face_zp);
        add_flux(graph_iter_zm, flux_rho_face_zm, flux_rhov_face_zm);
...
template<class Tvec, class TgridVec>
void shammodels::basegodunov::modules::NodeSumFluxDust<Tvec, TgridVec>::_impl_evaluate_internal() {
...
template class shammodels::basegodunov::modules::NodeSumFluxDust<f64_3, i64_3>;
