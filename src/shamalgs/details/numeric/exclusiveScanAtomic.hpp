...
namespace shamalgs::numeric::details {
            ...
            cgh.parallel_for<KernelExclusiveSumAtomicSync<T, group_size>>(
                ...
                [=](sycl::nd_item<1> id) {
                    ...
                    if (group_id.is_main_thread) {
                        ...
                        sycl::atomic_ref atomic_counter = device_counter.attach_atomic<sycl::memory_order_acq_rel>();
                        sycl::atomic_ref atomic_sum = global_sum.attach_atomic<sycl::memory_order_relaxed>();
                ...
                [=](sycl::nd_item<1> id) {
                    ...
                    if (group_id.is_main_thread) {
                        ...
                        if (group_id.dyn_group_id == 0) {
                            ...
                            T exclusive_group_prefix_sum = atomic_ref_T(acc_gsum[group_id.dyn_group_id-1]).load();

                            atomic_ref_T(acc_gsum[group_id.dyn_group_id]).store(exclusive_group_prefix_sum + group_sum);
    ...
    template<class T, u32 group_size>
    sycl::buffer<T> exclusive_sum_atomic_decoupled_v2(sycl::queue &q, sycl::buffer<T> &buf1, u32 len) {
    ...
    template<class T, u32 group_size>
    sycl::buffer<T> exclusive_sum_atomic_decoupled_v3(sycl::queue &q, sycl::buffer<T> &buf1, u32 len) {
    ...
    template<class T, u32 group_size>
    sycl::buffer<T> exclusive_sum_atomic_decoupled_v4(sycl::queue &q, sycl::buffer<T> &buf1, u32 len) {
