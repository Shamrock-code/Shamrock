    ...
    void NodeComputeCoordinates<Tvec, TgridVec>::_impl_evaluate_internal() {
        ...
        sham::distributed_data_kernel_call(  
            ...
            [this, one_over_Nside, dxfact](  
                ...
                Tvec offset; // offset of the lower left border of the cell from the lower left corner of the block
                for (u32 d = 0; d < dim; d++) {  
                    offset[d] = coord_array[d] * block_cell_size;
                }

                // so the coordinate of the cell center is pos_block_min + offset + half a cell  
                cell_coord[i] =  pos_block_min + offset + 0.5 * block_cell_size; // coordinates of the cell center  
            });  
        }  
