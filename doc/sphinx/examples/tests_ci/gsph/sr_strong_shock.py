...
rho_ana = np.zeros_like(x)
vx_ana = np.zeros_like(x)
P_ana = np.zeros_like(x)

for i in range(len(x)):
    rho_ana[i], vx_ana[i], P_ana[i] = sr_sample_solution(
        x[i], t_target, x0, P_L, n_L, 0.0, P_R, n_R, 0.0, gamma, star_state
    )

# Compute L2 errors (normalized)
err_rho = np.sqrt(np.mean((rho_sim - rho_ana) ** 2)) / np.mean(rho_ana)
...
# Regression testing with strict tolerance
expect_rho = 0.08
expect_vx = 0.12
expect_vy = 1e-10
expect_vz = 1e-10
