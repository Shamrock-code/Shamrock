...
This is the standard Sod problem in the relativistic regime.
"""
...
def sr_shock_velocity(P_star, P_a, rho_a, v_a, gamma, is_left):
    ...
    Returns (v_star, rho_star) behind the shock.
    """
    h_a = sr_enthalpy(P_a, rho_a, gamma)

    # Quadratic for h_star (Pons et al. Eq. 27)
    ...
    # Lorentz factor (for v_t=0)
    gamma_a = 1.0 / np.sqrt(1.0 - v_a**2) if abs(v_a) < 0.9999 else 1e4
    N_a = rho_a * gamma_a  # Lab-frame density
...
def sr_rarefaction_velocity(P_star, P_a, rho_a, v_a, gamma, is_left):
    """
    Compute velocity across isentropic rarefaction (v_t=0 analytical solution).
    Uses Marti & Muller analytical formula.
    """
    # Isentropic relation: P/rho^gamma = const
...
def sr_solve_riemann(P_L, rho_L, v_L, P_R, rho_R, v_R, gamma):
    ...
    Uses Brent's method to find the root of v_L(P) - v_R(P) = 0.
    """
    def residual(P):
    ...
    try:
        ...
        # Fallback: try different bracket
        P_star = 0.5 * (P_L + P_R)

    v_star = sr_wave_curve(P_star, P_L, rho_L, v_L, gamma, True)

    # Get star region densities
    if P_star > P_L:
...
def sr_characteristic_speed(v, cs, sign):
    """Relativistic characteristic speed: (v + sign*cs)/(1 + sign*v*cs)"""
    return (v + sign * cs) / (1.0 + sign * v * cs)


def sr_sample_rarefaction(xi, P_a, rho_a, v_a, gamma, is_left):
    """Sample state inside rarefaction fan at self-similar variable xi = x/t."""
    K_entropy = P_a / (rho_a**gamma)
    ...
    def residual(P):
        ...
        # Compute velocity from Riemann invariant
        sign = 1.0 if is_left else -1.0
        sqrt_gm1 = np.sqrt(gamma - 1.0)
...
# =============================================================================
# Simulation Parameters (matching Kitajima et al. 2025, Section 3.1.1)
# =============================================================================
gamma = 5.0 / 3.0
n_L, n_R = 1.0, 0.125
P_L, P_R = 1.0, 0.1
...
# Paper uses 3200 particles left, 400 particles right (equal baryon number per particle)
# Ratio 3200:400 = 8:1, matching density ratio 1.0:0.125 = 8:1
resol = 100  # base resolution

# Spacing factor for density discontinuity
fact = (n_L / n_R) ** (1.0 / 3.0)

print("Setting up SR-GSPH Sod Shock Tube...")
...
# Paper uses t=0.35 for Sod problem
t_target = 0.35
print(f"Running SR-GSPH Sod Shock Tube (M4, HLLC, t={t_target})...")
...
print("SR-GSPH Sod Shock Tube completed.")
print(f"Particles in simulation: {len(x)}")
print(f"x range: [{x.min():.4f}, {x.max():.4f}]")
print(f"vx range: [{vx.min():.4f}, {vx.max():.4f}]")
print(f"rho range: [{rho_sim.min():.4f}, {rho_sim.max():.4f}]")
...
u_exact = p_exact / ((gamma - 1) * rho_exact)
print("Exact solution computed successfully")

# Create Kitajima-style plots (4 vertically stacked subplots)
output_dir = "/Users/guo/Downloads/sph-simulators/simulations_data/sr_sod"
...
plt.tight_layout()
plot_path = os.path.join(output_dir, "sr_sod_shock_tube.png")
plt.savefig(plot_path, dpi=150, bbox_inches='tight')
print(f"\nPlot saved to: {plot_path}")

print("\n" + "=" * 50)
print("SR-GSPH Sod Shock Tube Test Complete")
print("=" * 50)
