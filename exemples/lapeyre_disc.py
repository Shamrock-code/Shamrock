...
outputdir = '/Users/ylapeyre/Documents/Shamwork/'
...
dump = model.do_vtk_dump(outputdir+"initdump2.vtk", False)

print("Run")

model.evolve_once_override_time(0,0)

dump2 = model.do_vtk_dump(outputdir+"finaldump2.vtk", False)
...
while next_dt_target <= t_target:
    ...
    fname_phdump = outputdir + "dump_{:04}".format(i_dump)
    model.evolve_until(next_dt_target)

    model.do_vtk_dump(fname, True)
    dump = model.make_phantom_dump()
    dump.save_dump(fname_phdump)
