...
/**
 * @file SRIterateSmoothingLength.cpp
...
namespace shammodels::gsph::physics::sr {
    ...
    void SRIterateSmoothingLength<Tvec, SPHKernel>::_impl_evaluate_internal() {
            ...
            [h_evol_max      = this->h_evol_max,
             ...
             c_smooth        = this->c_smooth](
                ...
                if (eps[id_a] > Tscal(1e-6)) {
                    ...
                    // Kitajima Eq. 232-233: Use C_smooth x h for computing V_p*
                    // V_p*(x) = [Sum_j W(x-x_j, C_smooth x h(x))]^(-1)
                    ...
                    // V_p* = [Sum_j W(r_ij, C_smooth x h_i)]^(-1)
                    // h_i = eta x V_p*^(1/dim)
                    // This is implicitly coupled, so we iterate until convergence.
                    // IMPORTANT: Sum_j includes self (j=i), so add self-contribution W(0, h)
                    ...
                    particle_looper.for_each_object(id_a, [&](u32 id_b) {
                        ...
                        // Use C_smooth x h for cutoff (Kitajima Eq. 232-233)
                        if (rab2 > dint) {
                            return;
                        }
                        ...
                        // Volume-based gather: use C_smooth x particle's h (Kitajima Eq. 232)
                        // W_sum = Sum_j W(r_ij, C_smooth x h_i)
                        W_sum += SPHKernel::W_3d(rab, h_smooth);
                    });

                    // Direct h computation from volume (Kitajima Eq. 221, 230-231):
                    // V = 1/W_sum, h = eta x V^(1/d) = eta / W_sum^(1/d)
                    // In Shamrock, eta = hfact (= hfactd)
    ...
    template<class Tvec, class SPHKernel>
    std::string SRIterateSmoothingLength<Tvec, SPHKernel>::_impl_get_tex() const {
        return R"(
\textbf{SR volume-based smoothing length (Kitajima et al. 2025)}

Volume from kernel summation:
\[
V_p^* = \left[ \sum_j W(|\mathbf{r}_p - \mathbf{r}_j|, C_{\text{smooth}} \cdot h_p) \right]^{-1}
\]

Direct $h$ computation:
\[
h_p = \eta \cdot V_p^{*1/d}
\]

where $C_{\text{smooth}}$ smooths $h$ variation across discontinuities.
)";
    }

    // Explicit template instantiations
    template class SRIterateSmoothingLength<f64_3, shammath::M4<f64>>;
    template class SRIterateSmoothingLength<f64_3, shammath::M6<f64>>;
    template class SRIterateSmoothingLength<f64_3, shammath::M8<f64>>;

    template class SRIterateSmoothingLength<f64_3, shammath::C2<f64>>;
    template class SRIterateSmoothingLength<f64_3, shammath::C4<f64>>;
    template class SRIterateSmoothingLength<f64_3, shammath::C6<f64>>;

    template class SRIterateSmoothingLength<f64_3, shammath::TGauss3<f64>>;
