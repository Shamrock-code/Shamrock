...
namespace shammodels::gsph::modules {
    ...
    void VTKDump<Tvec, SPHKernel>::do_dump(std::string filename, bool add_patch_world_id) {
        ...
        const u32 ivxyz  = pdl.get_field_idx<Tvec>(shammodels::gsph::names::newtonian::vxyz);
        const u32 iaxyz  = pdl.get_field_idx<Tvec>(shammodels::gsph::names::newtonian::axyz);
        const u32 ihpart = pdl.get_field_idx<Tscal>(shammodels::gsph::names::common::hpart);

        // Check for optional internal energy field
        const bool has_uint = solver_config.has_field_uint();
        const u32 iuint
            = has_uint ? pdl.get_field_idx<Tscal>(shammodels::gsph::names::newtonian::uint) : 0;
        ...
        // Use pre-computed fields from storage
        vtk_dump_add_solvergraph_field(scheduler(), writter, *storage.density, "rho");
        vtk_dump_add_solvergraph_field(scheduler(), writter, *storage.pressure, "P");
        vtk_dump_add_solvergraph_field(scheduler(), writter, *storage.soundspeed, "cs");
    }
