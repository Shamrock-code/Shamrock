            ...
            auto e = q.submit(depends_list, [&, use_grad_h, has_pmass, hfact, n_real](sycl::handler &cgh) {
                ...
                shambase::parallel_for(
                    cgh, pdat.get_obj_cnt(), "SR-GSPH derivs exact", [=](u64 gid) {
                        ...
                        const Tscal P_restframe_a = (gamma_eos - Tscal{1}) * n_restframe_a * u_restframe_a;

                        // Specific enthalpy: H = 1 + u/c² + P/(nc²) (rest-frame quantities)
                        const Tscal H_a = Tscal{1} + u_restframe_a / c2 + P_restframe_a / (n_restframe_a * c2);
                        ...
                        particle_looper.for_each_object(id_a, [&](u32 id_b) {
                            ...
                            const Tscal P_restframe_b = (gamma_eos - Tscal{1}) * n_restframe_b * u_restframe_b;

                            const Tscal H_b = Tscal{1} + u_restframe_b / c2 + P_restframe_b / (n_restframe_b * c2);
