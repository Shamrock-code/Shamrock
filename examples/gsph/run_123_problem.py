...
Two identical bodies of gas move apart from a common interface at high
speed (:math:`\\rho_L=\\rho_R=1`, :math:`u_L=-2`, :math:`u_R=+2`,
:math:`p_L=p_R=0.4`). This is Toro's standard "Test 2" benchmark: both
waves are rarefactions, and they drive the star-region pressure to near
vacuum (:math:`p^*/p_L\\approx 4.7\\times10^{-3}`). It is the classical
stress test for solver robustness near vacuum conditions, where
linearized/approximate Riemann solvers are prone to returning negative
pressure or density.
"""
...
# velocities on each half (no density or pressure discontinuity).
...
print(f"123 problem: p* = {p_star:.6g} (p*/p_L = {p_star / P_L:.4g}), u* = {u_star:.6g}")
