...
namespace shammodels::basegodunov {
    ...
    /**
     * @brief get the time integrator config set at runtime. MUSCL scheme by default.
     */
    struct TimeIntegratorConfig {
        TimeIntegratorMode time_integrator_config = MUSCL;
        inline bool is_muscl_scheme() {
            if (time_integrator_config == MUSCL)
                return true;
            return false;
        }
    };
