...
namespace sham {
    ...
    inline void distributed_data_kernel_call(
        ...
        RefIn in,
        RefOut in_out,
        shambase::DistributedData<index_t> thread_counts,
        Functor &&func,
        Targs... args) {

        auto mrefs_in = thread_counts.template map<decltype(in.get(0))>([&](u64 id, index_t &n) {
            logger::debug_ln("kern call", "build multi ref in for patch", id);
            return in.get(id);
        });
        auto mrefs_in_out
            = thread_counts.template map<decltype(in_out.get(0))>([&](u64 id, index_t &n) {
                  logger::debug_ln("kern call", "build multi ref in_out for patch", id);
                  return in_out.get(id);
              });

        thread_counts.for_each([&](u64 id, index_t &n) {
            logger::debug_ln(
