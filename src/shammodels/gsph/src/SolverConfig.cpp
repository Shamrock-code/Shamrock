...
void shammodels::gsph::SolverConfig<Tvec, SPHKernel>::set_layout(
    ...
    // Position
    pdl.add_field<Tvec>(fields::newtonian::xyz, 1);

    // Velocity
    pdl.add_field<Tvec>(fields::newtonian::vxyz, 1);

    // Acceleration
    pdl.add_field<Tvec>(fields::newtonian::axyz, 1);

    // Smoothing length
    pdl.add_field<Tscal>(fields::newtonian::hpart, 1);

    // Internal energy (for adiabatic EOS)
    if (has_field_uint()) {
        pdl.add_field<Tscal>(fields::newtonian::uint, 1);
        pdl.add_field<Tscal>(fields::newtonian::duint, 1);
    }
}
...
void shammodels::gsph::SolverConfig<Tvec, SPHKernel>::set_ghost_layout(
    ...
    ghost_layout.add_field<Tvec>(fields::newtonian::vxyz, 1);

    // Smoothing length
    ghost_layout.add_field<Tscal>(fields::newtonian::hpart, 1);

    // Omega (grad-h correction)
    ghost_layout.add_field<Tscal>(fields::newtonian::omega, 1);

    // Density (computed via SPH summation)
    ghost_layout.add_field<Tscal>(fields::newtonian::density, 1);

    // Internal energy (for adiabatic EOS)
    if (has_field_uint()) {
        ghost_layout.add_field<Tscal>(fields::newtonian::uint, 1);
    }
}
