...
namespace shammath {
    ...
    template<class Tvec>
    const DustConsState<Tvec>& DustConsState<Tvec>::operator*=(const typename DustConsState<Tvec>::Tscal factor)
    {
        rho    *= factor;
        rhovel *= factor;
        return *this;
    ...
    template<class Tvec>
    inline constexpr DustPrimState<Tvec>
    d_cons_to_prim(const DustConsState<Tvec> d_cons)
    {
    ...
    template<class Tvec>
    inline constexpr DustConsState<Tvec>
    d_hydro_flux_x(const DustConsState<Tvec> d_cons)
    {
...
//Huang & Bai , A Multiﬂuid Dust Module in Athena++: Algorithms and Numerical Tests, 2022
// Equation (32)
    template<class Tcons>
    inline constexpr auto
    huang_bai_flux_x(Tcons cL, Tcons cR)
    {
        ...
        if(d_primL[0] > 0 && d_primR[0] > 0)
            d_flux = fL;
        else if(d_primL[0] < 0 && d_primR[0] < 0)
            d_flux = fR;
        else if(d_primL[0] < 0 && d_primR[0] > 0)
            d_flux *=0;
        else if(d_primL[0] > 0 && d_primR[0] < 0)
    ...
    {
        return x_to_z(d_hll_flux_x(d_z_to_x(cL), d_z_to_x(cR)));
    }
