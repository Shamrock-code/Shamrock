...
void shammodels::basegodunov::modules::ComputeGradient<Tvec, TgridVec>::
    _compute_grad_v_dust_van_leer(){
        ...
        shamrock::SchedulerUtility utility{scheduler()};
        shamrock::ComputeField<Tvec> resultx = utility.make_compute_field<Tvec>("gradient dx v dust", (ndust * AMRBlock::block_size), [&](u64 id)
        {
            return storage.merged_patchdata_ghost.get().get(id).total_elements;
        });

        shamrock::ComputeField<Tvec> resulty = utility.make_compute_field<Tvec>("gradient dy v dust", (ndust * AMRBlock::block_size), [&](u64 id)
        {
            return storage.merged_patchdata_ghost.get().get(id).total_elements;
        });

         shamrock::ComputeField<Tvec> resultz = utility.make_compute_field<Tvec>("gradient dz v dust", (ndust * AMRBlock::block_size), [&](u64 id)
        {
            return storage.merged_patchdata_ghost.get().get(id).total_elements;
        });
